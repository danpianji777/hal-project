...
class ServerThread extends Thread {
    ...
    /* 2012-5-9 */
    /* support the YPbPr */
    private boolean mDeviceHasYpbpr = false;
    /* add by Gary. end   -----------------------------------}} */
    ...
    public void run() {
        ...
        /* add by cjcheng start {----------------------------------- */
        /* support Internal 3G 2013-04-24 */
        boolean internal_3g_en = (Settings.System.getInt(context.getContentResolver(), Settings.System.INTERNAL_3G_CTRL, 0) != 0);
        if (internal_3g_en){
            Log.d(TAG, "set internal 3g enabled");
            Gpio.writeGpio('i', 0, 1);
            Gpio.writeGpio('i', 1, 1);
        }
        /* add by cjcheng end   -----------------------------------} */
        /* add by Gary. start {{----------------------------------- */
        /* 2011-12-10 */
        ...
        int output_type = DispInfo.getDispType();
        int output_mode = DispInfo.getDispMode();
		boolean product_lvds = false;
        Slog.e(TAG, "disp_type = " + output_type);
        Slog.e(TAG, "disp_mode = " + output_mode);

        if (output_type == 1 || output_type == 8)
            product_lvds = true;

    if (!product_lvds) { // add by cjcheng
        Slog.e(TAG,"cjcheng enter tv mode");
	        /*
		...
		String savedFormatName = null;
//		String savedFormatName = Settings.System.getString(context.getContentResolver(),
//           		                                       Settings.System.DISPLY_OUTPUT_FORMAT);
		if (output_type == 2){
			if (output_mode == 11)
		   		savedFormatName = DispList.DISP_FORMAT_NAME_PAL;
			else if (output_mode == 14)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_NTSC;
			else if (output_mode == 0)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_YPBPR_480I;
			else if (output_mode == 1)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_YPBPR_576I;
			else if (output_mode == 2)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_YPBPR_480P;
			else if (output_mode == 3)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_YPBPR_576P;
			else if (output_mode == 4)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_YPBPR_720P_50HZ;
			else if (output_mode == 5)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_YPBPR_720P_60HZ;
			else if (output_mode == 9)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_YPBPR_1080P_50HZ;
			else if (output_mode == 10)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_YPBPR_1080P_60HZ;
//			else
//		    		savedFormatName = DispList.DISP_FORMAT_NAME_YPBPR_720P_60HZ;
		}
		if (output_type == 4) {
			if (output_mode == 0)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_HDMI_480I;
			else if (output_mode == 1)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_HDMI_576I;
			else if (output_mode == 2)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_HDMI_480P;
			else if (output_mode == 3)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_HDMI_576P;
			else if (output_mode == 4)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_HDMI_720P_50HZ;
			else if (output_mode == 5)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_HDMI_720P_60HZ;
			else if (output_mode == 6)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_HDMI_1080I_50HZ;
			else if (output_mode == 7)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_HDMI_1080I_60HZ;
			else if (output_mode == 8)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_HDMI_1080P_24HZ;
			else if (output_mode == 9)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_HDMI_1080P_50HZ;
			else if (output_mode == 10)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_HDMI_1080P_60HZ;
//			else
//		    		savedFormatName = DispList.DISP_FORMAT_NAME_HDMI_720P_60HZ;
		}
		if (output_type == 8){
			if (output_mode == 6)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_VGA_640_480;
			else if ( output_mode == 5)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_VGA_800_600;
			else if (output_mode == 4)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_VGA_1024_768;
			else if (output_mode == 11)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_VGA_1280_720;
			else if (output_mode == 3)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_VGA_1280_1024;
			else if (output_mode == 2)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_VGA_1360_768;
			else if (output_mode == 1)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_VGA_1440_900;
			else if (output_mode == 0)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_VGA_1680_1050;
			else if (output_mode == 10)
		    		savedFormatName = DispList.DISP_FORMAT_NAME_VGA_1920_1080;
//			else
//		    		savedFormatName = DispList.DISP_FORMAT_NAME_VGA_1280_720;
		}
        ...
        if (ypbprIsConnected && (DispInfo.getDispMode() < 11))
            mDeviceHasYpbpr = true; 
        ...
        if(curFormat.mOutputType != finalFormat.mOutputType || curFormat.mFormat != finalFormat.mFormat){
            displayManager.setDisplayParameter(0, finalFormat.mOutputType, finalFormat.mFormat);
            displayManager.setDisplayMode(DisplayManagerAw.DISPLAY_MODE_SINGLE_VAR);
        }
